...
# 构建（跳过类型检查），这是构建的主要方式，因为项目中有遗留的类型错误
pnpm build-only
...
- **`main.ts`** — 创建 Vue 应用，安装 Pinia、Vue Router、bkui-vue（全量引入）、vue-i18n、XSS 过滤指令，注册全局组件 `AgIcon`、`IconButton`、`CopyButton`、`CardContainer`。
- **`App.vue`** — 根布局，使用 `BkNavigation`（上下导航），包含顶部菜单、语言切换、用户信息。路由变化时获取环境配置和用户信息，通过 Feature Flag 控制菜单显隐。
...
主要模块包括：`gateway`、`resource`、`stage`、`permission`、`mcp-server`、`mcp-market`、`plugin-manage`、`online-debugging`、`audit-log`、`access-log`、`monitor`、`backend-services`、`ai`、`docs` 等。
